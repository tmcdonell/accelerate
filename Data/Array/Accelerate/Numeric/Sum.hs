...
-- TLM: Also, since we have allowed unsafe operations on floating point values
-- in the optimiser, these methods can be undone by the code generator... We
-- might need to explicitly disable optimisations for these instructions some
-- how, or implement them via the FFI to ensure these instructions are not
-- marked unsafe.
--
module Data.Array.Accelerate.Numeric.Sum (
...
kbnAdd :: (Num a, Ord a) => Exp (KBN a) -> Exp (KBN a) -> Exp (KBN a)
kbnAdd (unlift -> KBN s1 c1) (unlift -> KBN s2 c2) = lift (KBN s' c')
  where
    s' = s1 + s2
    c' = c1 + c2 + if abs s1 >= abs s2
                     then (s1 - s') + s2
                     else (s2 - s') + s1
...
kb2Add :: (Num a, Ord a) => Exp (KB2 a) -> Exp (KB2 a) -> Exp (KB2 a)
kb2Add (unlift -> KB2 s1 c1 cc1) (unlift -> KB2 s2 c2 cc2) = lift (KB2 sum' c' cc')
  where
    sum'  = s1 + s2
    c'    = t + k
    cc'   = cc1 + cc2 + if abs t >= abs k
                          then (t - c') + k
                          else (k - c') + t
    t     = c1 + c2
    k     = if abs s1 >= abs s2
...
kahanAdd :: Num a => Exp (Kahan a) -> Exp (Kahan a) -> Exp (Kahan a)
