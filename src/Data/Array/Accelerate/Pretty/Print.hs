...
prettyPreOpenAcc ctx prettyAcc extractAcc aenv pacc =
  case pacc of
    ...
    -- Collect min max i s     -> "collect"      .$ [ppE min, ppL max, ppL i] ) 2
  ...
  where
    ...
    -- ppL :: Maybe (PreExp acc aenv t) -> Adoc
    -- ppL = maybe "infinity" ppE
...
prettyPreOpenExp ctx prettyAcc extractAcc env aenv exp =
  case exp of
    ...
    IndexTrans sh         -> ppF1 "indexTrans"  (ppE sh)
    ...
    ToSlice _ sh ix       -> ppF2 "toSlice"     (ppE sh) (ppE ix)
